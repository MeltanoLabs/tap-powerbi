# AGENTS.md - AI Agent Development Guide for tap-powerbi
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from PowerBI.

### Key Components

1. **Tap Class** (`tap_powerbi/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_powerbi/client.py`): Handles API communication and authentication
1. **Streams** (`tap_powerbi/streams.py`): Define data streams and their schemas
   1. **Authentication** (`tap_powerbi/auth.py`): Implements OAuth2 authentication logic
...
1. Define stream class in `tap_powerbi/streams.py`
...
```python
class MyNewStream(PowerBIStream):
    name = "my_new_stream"
...
```bash
tap-powerbi --config config.json --discover
tap-powerbi --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_powerbi/tap.py`
...
```bash
# .env.example
TAP_POWERBI_API_URL=https://api.example.com
TAP_POWERBI_API_KEY=your_api_key_here
TAP_POWERBI_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-powerbi --version`), Python version, and a minimal reproduction case when reporting bugs.
